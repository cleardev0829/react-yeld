...
(function (root, factory) {
	...
	(function ($) {
		...
		var REGEXP_ENTER_OR_SPACE = new RegExp(
		...
		Selectpicker.prototype = {
				...
				function scroll(scrollTop, init) {
					...
					for (var i = 0; i < chunkCount; i++) {
						...
						if (
							currentChunk === undefined &&
							scrollTop <=
								that.selectpicker.current.data[end_of_chunk - 1]
									.position -
									that.sizeInfo.menuInnerHeight
						) {
							currentChunk = i;
					...
					if (that.activeIndex !== undefined) {
						prevActive =
							that.selectpicker.current.elements[
								that.selectpicker.current.map.newIndex[
						...
						if (init) {
							if (that.activeIndex !== that.selectedIndex) {
								...
								if (active.firstChild)
									active.firstChild.classList.remove(
										"active",
									);
							}
					...
					if (!that.options.liveSearch) {
						...
						if (that.selectpicker.view.currentActive) {
							that.selectpicker.view.currentActive.classList.remove(
								"active",
							);
			...
			createLi: function () {
				...
				var generateText = function (options) {
					...
					if (options.optionContent) {
						...
						if (options.optionSubtext) {
							...
							optionSubtextElement.innerHTML =
								options.optionSubtext;
				...
				var generateLabel = function (options) {
					var labelTextElement =
							elementTemplates.text.cloneNode(false),
						labelSubtextElement,
						labelIconElement;
					...
					if (options.labelIcon) {
						...
						labelIconElement.className =
							that.options.iconBase + " " + options.labelIcon;
				...
				$selectOptions.each(function (index) {
					...
					if (isOptgroup && thisData.divider !== true) {
						...
						if (
							(that.options.hideDisabled && isDisabled) ||
							thisData.hidden === true
						) {
							liIndex--;
							return;
						...
						mainElements.push(
							generateLI(false, index, classNames.DIVIDER),
						);
						mainData.push({
							type: "divider",
							originalIndex: index,
						...
						mainElements.push(
							generateLI(
								generateA(textElement, optionClass, inline),
								index,
							),
						);
						mainData.push({
				...
				this.selectpicker.view.availableOptionsCount =
					availableOptionsCount; // faster way to get # of available options without filter
			},
			...
			render: function () {
				...
				for (
					var i = 0, len = this.selectpicker.main.elements.length;
					i < len;
					i++
				) {
					...
					if (option && option.selected) {
						...
						if (
							(selectedItemsInTitle.length < 100 &&
								that.options.selectedTextFormat !== "count") ||
							...
							var thisData = this.selectpicker.main.data[i].data,
								icon =
									thisData.icon && that.options.showIcon
										? '<i class="' +
										  that.options.iconBase +
										  " " +
										  thisData.icon +
							...
							if (option.title) {
								...
								titleItem =
									icon + option.innerHTML.trim() + subtext;
							}
				...
				var title = !this.multiple
					? selectedItemsInTitle[0]
					: selectedItemsInTitle.join(this.options.multipleSeparator);
				...
				this.$button[0].title = htmlUnescape(
			...
			liHeight: function (refresh) {
				...
				a.className =
					"dropdown-item " +
					this.$element.find("option")[0].className;
				...
				if (this.options.dropdownAlignRight === "auto") {
					this.$menu.toggleClass(
						...
						this.sizeInfo.selectOffsetLeft >
							this.sizeInfo.selectOffsetRight &&
							this.sizeInfo.selectOffsetRight <
								this.$menu[0].offsetWidth - selectWidth,
					);
				}
			...
			setSize: function (refresh) {
				...
				if (refresh) {
					...
					selectedIndex =
						that.selectpicker.main.map.newIndex[
			...
			setOptionStatus: function () {
				...
				if (
					...
					for (
						...
						if (option) {
							var liIndex =
									this.selectpicker.main.map.newIndex[index],
			...
			setSelected: function (index, selected, liIndex, li) {
				...
				if (!keepActive) {
					if (
						...
						prevActive =
							this.selectpicker.main.elements[prevActiveIndex];

						prevActive.classList.toggle("selected", selected);
						prevActive.classList.remove("active");
						if (prevActive.firstChild) {
							prevActive.firstChild.classList.toggle(
								"selected",
								selected,
							);
							prevActive.firstChild.classList.remove("active");
				...
				this.$menuInner.on("click", "li a", function (e, retainActive) {
					...
					if (
						...
						if (triggerChange) {
							if (
								(prevValue !=
									getSelectValues(that.$element[0]) &&
									that.multiple) ||
								(prevIndex !=
			...
			changeAll: function (status) {
				...
				for (
					var i = 0;
					i < this.selectpicker.current.elements.length;
					i++
				) {
			...
			keydown: function (e) {
				...
				if (isArrowKey) {
					...
					if (matches.length) {
						...
						liActive =
							that.selectpicker.current.elements[searchMatch];
						liActive.classList.add("active");
						if (liActive.firstChild)
							liActive.firstChild.classList.add("active");
